...
const PRICE_ZONES = {
  ...
  NO4: {
    // Nord (Fritatt for MVA i realiteten, men oppgitt her som snittpris)
    name: 'NO4 (Nord-Norge)',
...
const DEFAULT_CONSUMPTION = [
  2200, 1900, 1600, 1100, 800, 500, 400, 450, 700, 1000, 1500, 2000,
];
...
const App = () => {
  ...
  // --- STATE ---
  const [selectedZone, setSelectedZone] = useState('NO3');
  const [surcharge, setSurcharge] = useState(0); // Ny state for påslag
  ...
  const handleReset = () => {
    if (
      window.confirm(
        'Er du sikker? Dette vil slette dine endringer og tilbakestille til standard 2025-verdier.'
      )
    ) {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans p-4 md:p-8">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <header className="flex flex-col lg:flex-row justify-between items-center gap-6 border-b border-slate-200 pb-6">
          <div className="text-center lg:text-left">
            ...
            <p className="text-slate-500">
              Sammenlign "Strømstøtte" vs. "Norgespris" (2025-tall)
            </p>
          </div>
          ...
          <div className="flex flex-wrap justify-center items-end gap-4 bg-white p-4 rounded-lg shadow-sm border border-slate-200">
            ...
            <div className="flex flex-col">
              ...
              >
                {Object.entries(PRICE_ZONES).map(([key, zone]) => (
          ...
          <div className="p-6 bg-slate-100 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                Spotprisene inkluderer nå påslaget ditt ({surcharge} øre). Du
                kan fortsatt endre enkeltmåneder manuelt.
              </p>
            </div>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {data.map((row, index) => (
                  ...
                  >
                    <td className="px-4 py-3 font-medium bg-slate-50 group-hover:bg-slate-100 w-24">
                      {row.month}
                    </td>
                    ...
                    <td className="px-4 py-2">
                      <input
                        ...
                        className={`w-full max-w-[120px] px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 transition-shadow ${
                          row.spotPrice !==
                          PRICE_ZONES[selectedZone].prices[index] + surcharge
                        ...
                        title={
                          row.spotPrice !==
                          PRICE_ZONES[selectedZone].prices[index] + surcharge
                            ? 'Denne måneden er manuelt endret'
                            : ''
                        }
                        value={row.spotPrice}
                        onChange={(e) =>
                          handleInputChange(index, 'spotPrice', e.target.value)
                        }
                      />
                    </td>
                  </tr>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col justify-between">
            <div>
              <h3 className="text-slate-500 font-medium mb-1">
                Dagens ordning (Strømstøtte)
              </h3>
              <p className="text-xs text-slate-400 mb-4">
                Du betaler spotpris opp til taket
              </p>
            </div>
            <div>
              <p className="text-3xl font-bold text-slate-800">
                {Math.round(results.totalStromstotte).toLocaleString()} kr
              </p>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400 mb-4">
                Fast pris uansett marked
              </p>
            </div>
            <div>
              <p className="text-3xl font-bold text-blue-600">
                {Math.round(results.totalNorgespris).toLocaleString()} kr
              </p>
              <p className="text-xs text-slate-400 mt-2 border-t pt-2 border-slate-100">
          ...
          >
            <div>
              ...
              >
                {results.totalDifference > 0
                  ? 'Du SPARER på Norgespris'
                  : 'Du TAPER på Norgespris'}
              </h3>
            </div>
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={results.monthly}>
                  ...
                  />
                  <Tooltip
                    formatter={(value) => `${Math.round(value)} kr`}
                    contentStyle={{
                      borderRadius: '8px',
                      border: 'none',
                  ...
                  <Legend />
                  <Bar
                    dataKey="costStromstotte"
                    name="Strømstøtte"
                    fill="#94a3b8"
                    radius={[4, 4, 0, 0]}
                  />
                  <Bar
                    dataKey="costNorgespris"
                    name="Norgespris"
                    fill="#2563eb"
                    radius={[4, 4, 0, 0]}
                  />
        ...
        <div className="text-center text-slate-400 text-sm pb-8 space-y-2">
          <p>
            NB: Utregningene inkluderer ikke nettleie eller faste avgifter, kun
            energileddet.
          </p>
